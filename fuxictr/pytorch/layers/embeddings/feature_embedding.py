...
class FeatureEmbedding(nn.Module):
    ...
    def forward(self, X, feature_source=[], feature_type=[], flatten_emb=False):
        # print('X: ', X['user_emb'].shape, X['adgroup_emb'].shape)
...
class FeatureEmbeddingDict(nn.Module):
            ...
            if self.is_required(feature):
                if not (use_pretrain and use_sharing) and embedding_dim == 1:
                    ...
                    if feature_spec["type"] == "sequence":
                        # self.feature_encoders[feature] = layers.MaskedAveragePooling()
                        self.feature_encoders[feature] = layers.DoNothing() ###
                else:
                    feat_dim = feature_spec.get("embedding_dim", embedding_dim)
                    if feature_spec.get("feature_encoder", None): # seq类型数据的Encoder在此被赋值
    ...
    def _reduce_to_first_last_dims(tensor):
        """
        简化版本的函数，实现相同的功能。
        """
        if tensor.dim() < 2:
            raise ValueError("输入张量必须至少有两个维度")
        # 对第1到倒数第二个维度进行平均池化
        return tensor.mean(dim=tuple(range(1, tensor.dim() - 1)))
    ...
    def dict2tensor(self, embedding_dict, flatten_emb=False, feature_list=[], feature_source=[],
        ...
        if flatten_emb:
            ...
            feature_emb = torch.stack([_reduce_to_first_last_dims(x) for x in feature_emb_list if len(x.shape)>2], dim=1)
        return feature_emb
        ...
        for feature in inputs.keys():
            ...
            if feature in self.embedding_layers:
                if feature_spec["type"] == "numeric":
                    ...
                    # print('a: ', feature, inp.shape, embeddings.shape)
                elif feature_spec["type"] == "categorical":
                    inp = inputs[feature].long()
                    embeddings = self.embedding_layers[feature](inp)
                    # print('b: ', feature, inp.shape, embeddings.shape)
                elif feature_spec["type"] == "sequence":
                    inp = inputs[feature].long()
                    embeddings = self.embedding_layers[feature](inp)
                    # print('c: ', feature, inp.shape, embeddings.shape)
                elif feature_spec["type"] == "embedding":
                    inp = inputs[feature].float()
                    # print('e: ', dir(self.embedding_layers[feature]))
                    embeddings = self.embedding_layers[feature](inp)
                    # print('d: ', feature, inp.shape, embeddings.shape)
                else:
                    raise NotImplementedError
                if feature in self.feature_encoders: # 这里把序列数据整个压缩了，失去了序列性
                    # self.feature_encoders: 仅对sequence和embedding类型的变量生效
